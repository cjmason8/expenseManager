...
/**
 * Matches {@code migration.sql}: deterministic UUID from legacy {@code documents.id} (bigint),
 * using {@code md5('expensemanager.documents.pk:' || old_id::text)} then version-4-shaped layout.
 */
public final class LegacyDocumentIdMapping {
	...
	public static UUID uuidFromLegacyLong(long oldId) {
		...
		String s = h.substring(0, 8) + "-" + h.substring(8, 12) + "-4" + h.substring(12, 15) + "-a" + h.substring(16, 19) + "-" + h.substring(20, 32);
