				...
				if (bodyPart.isMimeType("text/html")) {
					...
					CollectionUtil.splitAndConvert(content, "\n").stream().forEach(line -> {
						if (line.indexOf("Total\tincome") != -1) {
							...
							found[1] = false;
							rentalPayment
								.setAdminFee(new BigDecimal(line.substring(line.indexOf("$") + 1).replace(")", "")));
							...
							if (paymentToOwner.compareTo(rentalPayment.getPaymentToOwner()) != 0) {
								...
								notification.setMessage("There was an unbalanced Rental Payment for Wodonga "
					...
					if (rentalPayment.getStatementFrom().getMonthValue() <= 6) {
						folder = (rentalPayment.getStatementFrom().getYear() - 1) + "-"
							+ rentalPayment.getStatementFrom().getYear();
					} else {
						folder = rentalPayment.getStatementFrom().getYear() + "-"
							+ (rentalPayment.getStatementFrom().getYear() + 1);
					}
