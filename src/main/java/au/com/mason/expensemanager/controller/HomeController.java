...
public class HomeController {
	...
	TransactionsDto getRecurring(@PathVariable String activeOrAll) throws Exception {
		...
		return new TransactionsDto(incomeController.convertList(allRecurringIncome),
			expenseController.convertList(allRecurringExpenses));
	}
	...
	private TransactionsForWeekDto getTransactionsForWeekDto(LocalDate localDate) throws Exception {
		...
		if (localDate.isEqual(DateUtil.getMonday(LocalDate.now()))) {
			transactionsForWeekDto
				.setUnpaidExpenses(expenseController.convertList(expenseService.getUnpaidBeforeWeek(localDate)));
		} else {
